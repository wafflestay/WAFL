...
contract WaffleStay is ERC20 {
    ...
    constructor(
        ...
        address _to,
        address[] memory allowedSigners) ERC20(_name, _symbol) {

        super._mint(_to, 1000000000000000000000000000);
        initSigners(allowedSigners);
    }

    function initSigners(address[] memory allowedSigners) internal {
    ...
    function freezeAndUnfreezeAccount(address account, bytes[] calldata signatures, uint lockTime) external {
        require(signatures.length == MAX_NUMBER_SIGNERS, "signatures length required");
        ...
        // check signatures
        require(verifyMultiSig(operationHash, signatures), "Invalid Signatures");
    ...
    function verifyMultiSig(
        ...
        for (uint8 i = 0; i < signatures.length; i++) {
            ...
            address signedAccount = ECDSA.recover(prefixedHash, signatures[i]);
